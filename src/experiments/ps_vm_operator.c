...
{
    return op == OP_TEST_EQ || op == OP_TEST_NE || op == OP_TEST_GT || op == OP_TEST_GE || op == OP_TEST_LT ||
           op == OP_TEST_LE;
}
...
{
    return op == OP_BIT_NOT || op == OP_BIT_AND || op == OP_BIT_OR || op == OP_BIT_XOR || op == OP_BIT_SHL ||
           op == OP_BIT_SHR;
}

bool ps_vm_is_op_boolean(ps_vm_opcode op)
{
    return op == OP_BOOL_NOT || op == OP_BOOL_AND || op == OP_BOOL_OR || op == OP_BOOL_XOR;
}
...
ps_error vm_exec_op_unary(ps_vm *vm, ps_vm_opcode op)
{
    ...
    case OP_BIT_NOT: // applicable to integer or unsigned values
        ...
        else if (a->value->type->base == PS_TYPE_UNSIGNED)
        {
            ...
            return PS_ERROR_EXPECTED_INTEGER_OR_UNSIGNED;
        break;
    default:
...
ps_error vm_exec_op_binary(ps_vm *vm, ps_vm_opcode op)
{
    ...
    if (a->kind == PS_SYMBOL_KIND_AUTO)
        ps_vm_auto_free(vm, a->name);
    if (op == OP_ADD || op == OP_SUB || op == OP_MUL || op == OP_DIV || op == OP_MOD || op == OP_BIT_AND ||
        op == OP_BIT_OR || op == OP_BIT_XOR || op == OP_BOOL_AND || op == OP_BOOL_OR)
    {
        if (a->value->type->base != PS_TYPE_INTEGER)
