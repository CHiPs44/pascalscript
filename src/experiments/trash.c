 ...
 *
 * @param text
 * @return int PS_ERROR_NONE | PS_ERROR_IDENTIFIER_TOO_LONG
 */
 ...
 *
 * @return ps_error PS_ERROR_NONE | PS_ERROR_OVERFLOW
 */
ps_error lexer_copy_integer_value(char *text, ps_token *token)
{
    ...
    {
        fprintf(stderr, "PS_ERROR_OVERFLOW %s %ld", text, val);
...
ps_error lexer_copy_real_value(char *text, ps_token *token)
{
    ...
    if (errno == ERANGE)
    {
        fprintf(stderr, "PS_ERROR_OVERFLOW %s %f", text, val);
        return PS_ERROR_OVERFLOW;
    }
 ...
 *
 * @return ps_error PS_ERROR_NONE | PS_ERROR_STRING_TOO_LONG
 */
ps_error lexer_copy_string_value(char *text, ps_token *token)
{
    ...
    {
        fprintf(stderr, "PS_ERROR_STRING_TOO_LONG %ld |%s|", len, text);
        return PS_ERROR_STRING_TOO_LONG;
    }
