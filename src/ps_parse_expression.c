    ...
    case PS_TOKEN_NOT:
        ...
        *expression = (ps_ast_node *)ps_ast_create_unary_operation(start_line, start_column, operator_unary, operand);
        if (*expression == NULL)
            RETURN_ERROR(PS_ERROR_OUT_OF_MEMORY)
        PARSE_END("OK UNARY")
    // *** Identifier: variable, constant, enumeration value, function ***
...
bool ps_parse_function_call_power(ps_compiler *compiler, ps_ast_block *block, ps_ast_node **arg1, ps_ast_node **arg2)
{
    ...
    ps_value value1 = {.allocated = false, .type = type1, .data = {0}};
    if (type1 == NULL || (!ps_value_is_number(&value1) && !ps_value_is_real(&value1)))
    {
    ...
    ps_value value2 = {.allocated = false, .type = type2, .data = {0}};
    if (type2 == NULL || (!ps_value_is_number(&value2) && !ps_value_is_real(&value2)))
    {
