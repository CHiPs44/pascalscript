...
bool
ps_function_write_text (ps_interpreter *interpreter, FILE *f, ps_value *value)
{
...
bool
ps_function_randomize (ps_interpreter *interpreter)
{
...
bool
ps_function_copy_value (ps_interpreter *interpreter, ps_value *from,
                        ps_value *to)
{
  ...
  // Integer => Unsigned?
  if (from->type->base == PS_TYPE_INTEGER
      && to->type->base == PS_TYPE_UNSIGNED)
    {
  ...
  // Unsigned => Integer?
  if (from->type->base == PS_TYPE_UNSIGNED
      && to->type->base == PS_TYPE_INTEGER)
    {
...
/** @brief Compute bitwise not for integer / unsigned, logical not for boolean,
 * neg for integer / real */
bool
ps_function_unary_op (ps_interpreter *interpreter, ps_value *value,
                      ps_value *result, ps_token_type token_type)
{
  result->type = value->type;
  // NB: with FPC, not(subrange) or not(enum) yields integer result without
  // range checking
...
bool
ps_function_binary_op (ps_interpreter *interpreter, ps_value *a, ps_value *bb,
                       ps_value *result, ps_token_type token_type)
{
  ps_value b;
  if (a->type->base == bb->type->base)
  ...
  else
    {
      ...
      if ((a->type->base == PS_TYPE_INTEGER
           && bb->type->base == PS_TYPE_UNSIGNED)
          || (a->type->base == PS_TYPE_UNSIGNED
              && bb->type->base == PS_TYPE_INTEGER))
        {
          b.type = a->type;
...
bool
ps_function_exec (ps_interpreter *interpreter, ps_symbol *symbol,
                  ps_value *value, ps_value *result)
{
...
bool
ps_function_odd (ps_interpreter *interpreter, ps_value *value,
                 ps_value *result)
{
...
bool
ps_function_even (ps_interpreter *interpreter, ps_value *value,
                  ps_value *result)
{
...
bool
ps_function_ord (ps_interpreter *interpreter, ps_value *value,
                 ps_value *result)
{
  switch (value->type->base)
    {
...
bool
ps_function_chr (ps_interpreter *interpreter, ps_value *value,
                 ps_value *result)
{
  ...
  switch (value->type->base)
    ...
    case PS_TYPE_INTEGER:
      if (interpreter->range_check
          && (value->data.i < 0 || value->data.i > PS_CHAR_MAX))
        {
          interpreter->error = PS_RUNTIME_ERROR_OUT_OF_RANGE;
          return false;
        }
...
bool
ps_function_pred (ps_interpreter *interpreter, ps_value *value,
                  ps_value *result)
{
  result->type = value->type;
  switch (value->type->base)
...
bool
ps_function_succ (ps_interpreter *interpreter, ps_value *value,
                  ps_value *result)
{
  result->type = value->type;
  switch (value->type->base)
...
bool
ps_function_random (ps_interpreter *interpreter, ps_value *value,
                    ps_value *result)
{
  if (value == NULL)
    {
...
bool
ps_function_abs (ps_interpreter *interpreter, ps_value *value,
                 ps_value *result)
{
  result->type = value->type;
  switch (value->type->base)
...
bool
ps_function_trunc (ps_interpreter *interpreter, ps_value *value,
                   ps_value *result)
{
  switch (value->type->base)
    {
    case PS_TYPE_REAL:
      if (interpreter->range_check
          && (value->data.r < PS_INTEGER_MIN
              || value->data.r > PS_INTEGER_MAX))
        {
          interpreter->error = PS_RUNTIME_ERROR_OUT_OF_RANGE;
          return false;
        }
...
bool
ps_function_round (ps_interpreter *interpreter, ps_value *value,
                   ps_value *result)
{
  switch (value->type->base)
    {
...
bool
ps_function_int (ps_interpreter *interpreter, ps_value *value,
                 ps_value *result)
{
  switch (value->type->base)
    {
...
bool
ps_function_frac (ps_interpreter *interpreter, ps_value *value,
                  ps_value *result)
{
  double int_part;
  switch (value->type->base)
