...
bool ps_visit_expression (ps_interpreter *interpreter, bool exec,
                          ps_value *result);
 ...
 */
bool
ps_visit_function_call (ps_interpreter *interpreter, bool exec,
                        ps_symbol *symbol, ps_value *result)
{
  USE_LEXER;
  ...
  if (exec)
    {
      if (!ps_function_exec (interpreter, symbol, null_arg ? NULL : &arg,
                             result))
 ...
 *  factor  =   '(' , expression , ')'
 *          |   identifier
 *          |   char_value | integer_value | unsigned_value | real_value |
 * boolean_value |   [ '+' | '-' | 'NOT' ] , factor
 *          ;
 */
bool
ps_visit_factor (ps_interpreter *interpreter, bool exec, ps_value *result)
{
  USE_LEXER;
  SET_VISITOR ("FACTOR");
  TRACE_BEGIN ("");
  ...
  switch (lexer->current_token.type)
    ...
    case PS_TOKEN_NOT:
      ...
      if (exec)
        {
          if (!ps_function_unary_op (interpreter, &factor, result,
                                     unary_operator))
            TRACE_ERROR ("UNARY");
        }
      break;
      ...
      if (exec)
        {
          ...
          result->data.s
              = ps_string_create (strlen (lexer->current_token.value.s),
 ...
 */
bool
ps_visit_term (ps_interpreter *interpreter, bool exec, ps_value *result)
{
  ...
  do
    {
      ...
      if (exec)
        {
          if (!ps_function_binary_op (interpreter, &left, &right, result,
 ...
 */
bool
ps_visit_simple_expression (ps_interpreter *interpreter, bool exec,
                            ps_value *result)
{
  static ps_token_type additive_operators[]
      = { PS_TOKEN_PLUS, PS_TOKEN_MINUS, PS_TOKEN_OR, PS_TOKEN_XOR };
  USE_LEXER;
 ...
 */
bool
ps_visit_expression (ps_interpreter *interpreter, bool exec, ps_value *result)
{
 ...
 */
bool
ps_visit_program (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("PROGRAM");
  TRACE_BEGIN ("");
  ...
  if (exec)
    {
      ps_symbol *program = ps_symbol_init (
          PS_SYMBOL_SCOPE_GLOBAL, PS_SYMBOL_KIND_PROGRAM, &identifier, NULL);
 ...
 */
bool
ps_visit_const (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("CONST");
  TRACE_BEGIN ("");
  ...
  do
    {
      ...
      switch (lexer->current_token.type)
        ...
        case PS_TOKEN_IDENTIFIER:
          constant
              = ps_symbol_table_get (interpreter->parser->symbols,
        ...
        case PS_TOKEN_INTEGER_VALUE:
          ...
          data.i = negate ? -lexer->current_token.value.i
                          : lexer->current_token.value.i;
          break;
        case PS_TOKEN_REAL_VALUE:
          type = ps_system_real.value->data.t;
          data.r = negate ? -lexer->current_token.value.r
                          : lexer->current_token.value.r;
          break;
        case PS_TOKEN_UNSIGNED_VALUE:
          if (negate)
            {
        ...
        case PS_TOKEN_STRING_VALUE:
          ...
          data.s = ps_string_create (strlen (lexer->current_token.value.s),
                                     lexer->current_token.value.s);
          if (data.s == NULL)
      ...
      if (exec)
        {
          ...
          if (value == NULL)
            RETURN_ERROR (PS_RUNTIME_ERROR_OUT_OF_MEMORY);
          constant
              = ps_symbol_init (PS_SYMBOL_SCOPE_GLOBAL,
                                PS_SYMBOL_KIND_CONSTANT, &identifier, value);
          if (constant == NULL)
            RETURN_ERROR (PS_RUNTIME_ERROR_OUT_OF_MEMORY);
          if (ps_symbol_table_add (interpreter->parser->symbols, constant)
              == NULL)
            RETURN_ERROR (PS_RUNTIME_ERROR_SYMBOL_NOT_ADDED);
        }
    }
 ...
 */
bool
ps_visit_var (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("VAR");
  TRACE_BEGIN ("");
      ...
      if (exec)
        {
          for (int i = 0; i <= var_count; i++)
            {
              value = ps_value_alloc (type, data);
              variable = ps_symbol_init (PS_SYMBOL_SCOPE_GLOBAL,
                                         PS_SYMBOL_KIND_VARIABLE,
                                         &identifier[i], value);
              if (variable == NULL)
                RETURN_ERROR (PS_RUNTIME_ERROR_OUT_OF_MEMORY);
              if (ps_symbol_table_add (interpreter->parser->symbols, variable)
                  == NULL)
                RETURN_ERROR (PS_RUNTIME_ERROR_SYMBOL_NOT_ADDED);
            }
        }
 ...
 */
bool
ps_visit_assignment (ps_interpreter *interpreter, bool exec,
                     ps_identifier *identifier)
{
  USE_LEXER;
  ...
  if (exec)
    {
      variable
          = ps_symbol_table_get (interpreter->parser->symbols, identifier);
      if (variable == NULL)
        {
 ...
 */
bool
ps_visit_write_or_writeln (ps_interpreter *interpreter, bool exec,
                           bool newline)
{
  USE_LEXER;
...
bool
ps_visit_assignment_or_procedure_call (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
 ...
 */
bool
ps_visit_compound_statement (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
 ...
 */
bool
ps_visit_if_then_else (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("IF_THEN_ELSE");
  ps_value result
      = { .type = ps_system_boolean.value->data.t, .data.b = false };
  TRACE_BEGIN ("");
 ...
 */
bool
ps_visit_repeat_until (ps_interpreter *interpreter, bool exec)
{
  ...
  do
    {
      ...
      if (!ps_buffer_read_next_char (lexer->buffer))
        RETURN_ERROR (
            PS_RUNTIME_ERROR_UNEXPECTED_TYPE); // TODO better error code
      READ_NEXT_TOKEN;
    }
  while (true);
 ...
 */
bool
ps_visit_while_do (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
 ...
 */
bool
ps_visit_for_do (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("FOR_DO");
      ...
      do
        {
          if (!ps_function_binary_op (
                  ...
                  step.data.i > 0 ? PS_TOKEN_LESS_OR_EQUAL
          ...
          if (!result.data.b)
            {
              ...
              if (!ps_visit_statement_or_compound_statement (interpreter,
                                                             false))
          ...
          if (!ps_buffer_read_next_char (lexer->buffer))
            RETURN_ERROR (
                PS_LEXER_ERROR_UNEXPECTED_EOF); // TODO better error code?
          READ_NEXT_TOKEN;
          ...
          if (step.data.i > 0)
            {
              if (!ps_function_succ (interpreter, variable->value,
                                     variable->value))
                TRACE_ERROR ("STEP/SUCC");
            }
          else
            {
              if (!ps_function_pred (interpreter, variable->value,
                                     variable->value))
                TRACE_ERROR ("STEP/SUCC");
            }
        }
 ...
 */
bool
ps_visit_statement (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
 ...
 */
bool
ps_visit_statement_list (ps_interpreter *interpreter, bool exec,
                         ps_token_type stop)
{
  USE_LEXER;
...
bool
ps_visit_statement_or_compound_statement (ps_interpreter *interpreter,
                                          bool exec)
{
  USE_LEXER;
 ...
 */
bool
ps_visit_block (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("BLOCK");
  TRACE_BEGIN ("");
  ...
  do
    {
      switch (lexer->current_token.type)
        {
        case PS_TOKEN_CONST:
          if (!ps_visit_const (interpreter, exec))
            TRACE_ERROR ("CONST");
          break;
        case PS_TOKEN_TYPE:
          RETURN_ERROR (PS_ERROR_NOT_IMPLEMENTED);
          // if (!ps_visit_type(interpreter, exec))
          //     TRACE_ERROR("TYPE");
          // break;
        case PS_TOKEN_VAR:
          if (!ps_visit_var (interpreter, exec))
            TRACE_ERROR ("VAR");
          break;
 ...
 */
bool
ps_visit_start (ps_interpreter *interpreter, bool exec)
{
  USE_LEXER;
  SET_VISITOR ("START");
  TRACE_BEGIN ("");
