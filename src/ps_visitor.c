 ...
 */
bool
ps_visit_function_call(ps_interpreter* interpreter,
                       bool exec,
                       ps_symbol* symbol,
                       ps_value* result)
{
 ...
 *          |   function_call
 *          |   nil
 *          |   string_value | char_value | integer_value | unsigned_value |
 * real_value | boolean_value |   [ '+' | '-' | 'NOT' ] , factor
 *          ;
 */
bool
  ...
  switch (lexer->current_token.type) {
    ...
    case PS_TOKEN_STRING_VALUE:
      if (exec) {
        ...
        result->data.s = ps_string_create(strlen(lexer->current_token.value.s),
  ...
  do {
    ...
    if (exec) {
      if (!ps_function_binary_op(
            interpreter, &left, &right, result, multiplicative_operator))
        TRACE_ERROR("BINARY");
 ...
 */
bool
ps_visit_simple_expression(ps_interpreter* interpreter,
                           bool exec,
                           ps_value* result)
{
  ...
  do {
    ...
    if (exec) {
      if (!ps_function_binary_op(
            interpreter, &left, &right, result, additive_operator))
        TRACE_ERROR("BINARY");
      left.type = result->type;
      left.data = result->data;
    }
...
ps_visit_expression(ps_interpreter* interpreter, bool exec, ps_value* result)
{
  ...
  if (exec) {
    result->type = ps_system_boolean.value->data.t;
    if (!ps_function_binary_op(
          interpreter, &left, &right, result, relational_operator))
      TRACE_ERROR("BINARY");
  }
  TRACE_END("2");
  return true;
...
ps_visit_const(ps_interpreter* interpreter, bool exec)
{
  ...
  do {
    ...
    switch (lexer->current_token.type) {
      case PS_TOKEN_IDENTIFIER:
        constant = ps_symbol_table_get(interpreter->parser->symbols,
      ...
      case PS_TOKEN_INTEGER_VALUE:
        ...
        data.i =
          negate ? -lexer->current_token.value.i : lexer->current_token.value.i;
        break;
      case PS_TOKEN_REAL_VALUE:
        type = ps_system_real.value->data.t;
        data.r =
          negate ? -lexer->current_token.value.r : lexer->current_token.value.r;
        break;
      case PS_TOKEN_UNSIGNED_VALUE:
        if (negate) {
          type = ps_system_integer.value->data.t;
    ...
    if (exec) {
      ...
      constant = ps_symbol_init(
        PS_SYMBOL_SCOPE_GLOBAL, PS_SYMBOL_KIND_CONSTANT, &identifier, value);
      if (constant == NULL)
        RETURN_ERROR(PS_RUNTIME_ERROR_OUT_OF_MEMORY);
 ...
 */
bool
ps_visit_assignment(ps_interpreter* interpreter,
                    bool exec,
                    ps_identifier* identifier)
{
  USE_LEXER;
 ...
 */
bool
ps_visit_write_or_writeln(ps_interpreter* interpreter, bool exec, bool newline)
{
  USE_LEXER;
...
bool
ps_visit_statement_list(ps_interpreter* interpreter,
                        bool exec,
                        ps_token_type stop);
bool
...
ps_visit_if_then_else(ps_interpreter* interpreter, bool exec)
{
  ...
  ps_value result = { .type = ps_system_boolean.value->data.t,
                      .data.b = false };
  TRACE_BEGIN("");
...
ps_visit_for_do(ps_interpreter* interpreter, bool exec)
{
  ...
  if (!exec) {
    ...
    // DOWNTO)
    do {
      if (!ps_function_binary_op(interpreter,
                                 variable->value,
                                 &finish,
                                 &result,
...
bool
ps_visit_statement_or_compound_statement(ps_interpreter* interpreter, bool exec)
{
  USE_LEXER;
