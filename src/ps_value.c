...
char *ps_value_get_enum(const ps_value *value)
{
    ...
    uint8_t index = (uint8_t)value->data.u;
...
#define ENUM_VALUE                                                                                                     \
    {                                                                                                                  \
        ...
        if (debug)                                                                                                     \
            snprintf(buffer, sizeof(buffer) - 1, "%s.%s (%u)", value->type->name, enum_value, value->data.u);          \
        else                                                                                                           \
            snprintf(buffer, sizeof(buffer) - 1, "%s", enum_value);                                                    \
        return buffer;                                                                                                 \
    }
